...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'username', 'is_superuser', 'email', 'password', 'password2']
        extra_kwargs = {
...
class GoogleLoginSerializer(serializers.Serializer):
    auth_code = serializers.CharField(label=_('Authorization code'), required=False, allow_blank=True)
    access_token = serializers.CharField(label=_('Access Token'))
    referer = serializers.EmailField(label=_('Referer'), required=False, allow_blank=True)
